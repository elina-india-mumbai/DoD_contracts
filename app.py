...
a single canonical term. DoD contracts only. Uses Total Obligated Amount
throughout (Award Amount is often 0 for contracts).
"""
...
def fetch_rsrt_awards(keyword, fy_start, fy_end, agencies, recipient_types, max_pages):
    """Pull contracts matching keyword in description."""
    ...
    for page in range(1, max_pages + 1):
        payload = {
            ...
            "limit": 100,
            "page": page,
            "sort": "Award Amount",   # leave as Award Amount; API rejects others
            "order": "desc",
        }
        try:
...
st.caption(
    "DoD contracts only (award type codes A, B, C, D). Uses Total Obligated "
    "Amount throughout (Award Amount is often 0 for contracts)."
)
...
st.success(
    ...
    f"FY{fy_start}–FY{fy_end} — DoD Higher-Ed Contracts"
)
...
st.caption(
    ...
    "the DoD contracts master file for grants-vs-contracts comparison."
)
